...
export const getRoomDetails = async (roomId) => {
  try {
    const response = await APIWithoutV1.get(`filter_room_city/rooms/${roomId}/`, {
      
    });
    return response.data;
  } catch (error) {
    console.error("Error getting Room Data:", error.response?.data || error.message);
    throw error;
  }
};
...
export const getCityDetails = async (city) => {
  try {
    const response = await API.get(`filter-rooms-by-city/`, {
      params: { city }
    });
    return response.data.rooms;
  } catch (error) {
    console.error("Error getting Room Data:", error.response?.data?.rooms || error.message);
    throw error;
  }
};

export const getAmenityFilterData = async (access, amenity_name, tenant) => {
  try {
    const response = await API.get(`${tenant}/amenity-rooms/`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${access}`,
      },
      // amenity_name: amenity_name,
      params: { amenity_name: amenity_name },
    });
    return response.data;
  } catch (error) {
    console.error("Error fetching balance details:", error);
...
export const getHotelData = async (access, tenant) => {
  try {
    const response = await API.get(`${tenant}/hotel_amenity/`);
    return response.data;
  } catch (error) {
    console.error("Error fetching balance details:", error);
    throw error;
...
export const getHotelFilterData = async () => {
  try {
    const response = await API.get(`/hotel_amenity/`);
    return response.data;
  } catch (error) {
    console.error("Error fetching balance details:", error);
    throw error;
...
export const getFilterData = async (amenityName = "") => {
  try {
    const response = await APIWithoutV1.get(`/filter_room_city/hotel_amenity/`,{
...
// export const getHotel = async (adults = 1, children = 0, rooms = 1) => {
//   try {
//     const response = await APIWithoutV1.get(`/filter_room_city/filterHotel/`, {
//       params: { 
//         adults: adults,
//         children: children,
//         rooms: rooms,
...
//   }
// };
export const getHotel = async ({ city, adults = 1, children = 0, rooms = 1 }) => {
  try {
    const response = await APIWithoutV1.get(`/filter_room_city/filterHotel/`, {
      params: {
        city,
